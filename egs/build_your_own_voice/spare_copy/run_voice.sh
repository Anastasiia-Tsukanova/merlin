...
setup_data=false
need_to_generate_train_labels=true
train_tts=true
run_tts=true

if test "$#" -ne 1; then
    voice_name="gottingen_voice"
else
    voice_name=$1
...
if [ "$train_tts" = true ]; then
    ...
    sed -i s#'prepare_labels=.*'#'prepare_labels="'$need_to_generate_train_labels'"'# 02_prepare_labels.sh
    sed -i s#'needtogeneratelabels=.*'#'needtogeneratelabels="'$need_to_generate_train_labels'"'# scripts/prepare_labels_from_txt_customized.sh
    if [ "$need_to_generate_train_labels" = true ]; then
