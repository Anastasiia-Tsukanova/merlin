...
echo "preparing full-contextual labels using eLiteHTS frontend..."
bash ${WorkDir}/scripts/prepare_labels_from_txt_customized.sh $inp_txt $lab_dir $global_config_file $train

status_prev_step=$?
if [ $status_prev_step -eq 1 ]; then
    echo "Preparation of full-contextual labels unsuccessful!!"
    echo "Please check scripts/prepare_labels_from_txt_customized.sh"
    exit 1
fi
