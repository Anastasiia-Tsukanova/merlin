...
# The original script was found there:
# http://cental.uclouvain.be/elitehts/v1/#_span_class_monospaced_implementation_of_a_client_in_perl_span
#
# I modified it to take output file name as an argument
# so the new syntax is:
# eLiteHTS_client_mod.pl infile.txt texts hts train/run outfile.lab
#
...
if($#ARGV != 4)
{
        print "eLiteHTS_client.pl file(.txt or .textgrid) resource_type(texts or textgrids) output_format (hts, dls, textgrid_hts) mode (train or run) outputfile(.lab)\n";
        exit(1);
}
...
print MYFILE $res3->decoded_content;
close(MYFILE);
